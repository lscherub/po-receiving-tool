...
export async function putReportMeta(id, meta) {
  return put(metaPathname(id), JSON.stringify(meta), {
    ...
    addRandomSuffix: false, allowOverwrite: true, cacheControlMaxAge: 0, token: blobToken(),
  });
}
...
export async function logReportAccess(id, entry) {
  ...
  if (!meta.firstAccessedAt && meta.accesses.length) meta.firstAccessedAt = meta.accesses[0].at;
  ...
  if (!meta.firstAccessedAt) meta.firstAccessedAt = entry.at;
...
export async function listReportMetas(limit = 300) {
  const blobs = await listAllReportBlobs();
  const metas = blobs.filter((b) => b.pathname && b.pathname.endsWith('.json'));
  metas.sort((a, b) => {
    const ta = a.uploadedAt ? new Date(a.uploadedAt).getTime() : 0;
    const tb = b.uploadedAt ? new Date(b.uploadedAt).getTime() : 0;
    return tb - ta;
  });
  const slice = metas.slice(0, Math.max(1, Math.min(limit || 300, 1000)));
...
export function toReportSummary(m) {
  const accesses = Array.isArray(m.accesses) ? m.accesses : [];
  const first = accesses.length ? accesses[0].at : (m.firstAccessedAt || null);
  return {
    id: m.id,
    ...
    date: m.date || '',
    selected: m.selected || '',
    storeNames: Array.isArray(m.storeNames) ? m.storeNames : [],
    recipients: Array.isArray(m.recipients) ? m.recipients : [],
    subject: m.subject || '',
    filename: m.filename || '',
    viewUrl: '/view/' + m.id,
    accessCount: typeof m.accessCount === 'number' ? m.accessCount : accesses.length,
    viewed: accesses.length > 0 || !!m.firstAccessedAt,
    firstAccessedAt: first,
    lastAccessedAt: m.lastAccessedAt || (accesses.length ? accesses[accesses.length - 1].at : null),
  };
}
